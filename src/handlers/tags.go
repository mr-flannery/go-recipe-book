...
func GetRecipeUserTagsHandler(w http.ResponseWriter, r *http.Request) {
	...
	tags, err := models.GetUserTagsByRecipeID(user.ID, recipeIDInt)
	if err != nil {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusInternalServerError)
		json.NewEncoder(w).Encode(TagResponse{Success: false, Error: "Failed to get user tags"})
		return
	}
	...
	response := struct {
		Tags []UserTagResponse `json:"tags"`
	}{
		Tags: make([]UserTagResponse, len(tags)),
	}

	for i, tag := range tags {
		response.Tags[i] = UserTagResponse{ID: tag.ID, Name: tag.Name}
	}
