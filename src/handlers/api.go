...
func APICreateRecipeHandler(w http.ResponseWriter, r *http.Request) {
	...
	slog.Info("Recipe created successfully via API", "title", recipe.Title, "author_id", adminID)
	
	// For now, we don't have the recipe ID from SaveRecipe, so we'll return 0
	// In a real implementation, you might want to modify SaveRecipe to return the ID
	sendJSONResponse(w, "Recipe created successfully", 0)
}
